...
%   ---
%   MFILE:   cbfreeze.m
%   VERSION: 1.2 (Feb 14, 2014) (<a href="matlab:web('http://www.mathworks.com/matlabcentral/fileexchange/authors/11258')">download</a>) 
...
%            Muniz. (Sep 02, 2009)
%   1.2      Fixed BUG with colorbar title position. Thanks to Chris
%            Rapson. (Feb 14, 2014) 
...
for icb = 1:length(CBH)
 ...
 switch S
  ...
  otherwise % 'on'
   ...
   ivert = ismember(cb_prop.Location(1),['E' 'W']);
   
   % Gets colorbar label properties: Fixed BUG, Feb 2014
   if ivert
       cb_label = get(get(h,'YLabel'));
   else
       cb_label = get(get(h,'XLabel'));
   end
   ...
   % Removes read-only label properties:
   cb_label = rmfield(cb_label,{...
	 'Annotation',...
	 'BeingDeleted',...
	 'Extent',...
	 'Type'});
   ...
   % Recalculate label position based on new Lims
   if ivert
     old_XLim = cb_prop.XLim;
     new_XLim = get(h,'XLim');
     cb_label.Position = [new_XLim(1) + ...
       (cb_label.Position(1)-old_XLim(1))/(old_XLim(2)-old_XLim(1))...
       *(new_XLim(2)-new_XLim(1)), ...
       cb_label.Position(2),cb_label.Position(3)];
   else
     old_YLim = cb_prop.YLim;
     new_YLim = get(h,'YLim');
     cb_label.Position = [new_YLim(1) + ...
       (cb_label.Position(1)-old_YLim(1))/(old_YLim(2)-old_YLim(1))...
       *(new_YLim(2)-new_YLim(1)), ...
       cb_label.Position(2),cb_label.Position(3)];
   end
   ...
   set(h,cb_prop)
   if ivert
     set(get(h,'ylabel'),cb_label)
   else
     set(get(h,'xlabel'),cb_label)
   end
