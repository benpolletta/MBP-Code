function [bands,MI,MI_p_vals,MI_z_scores,MI_lz_scores,Binned_MI,Binned_MI_pt,Binned_MI_zt,Binned_MI_lzt,Binned_NPV,Binned_LPV]=CFC_September_fbank_beta(datafile,sampling_freq,min_cycles,nobins,noshufs,threshold,x_lims,x_bins,y_lims,y_bins,buttorder,pct_pass,spacing,units,outfilename,extradirname,opt_band)
...
if strcmp(spacing,'custom')==1
    [~,~,bands,~,~,A,P,F,cycle_bounds]=filter_fft_bank(data,sampling_freq,min_cycles,10,[1/signal_length sampling_freq/2],spacing,buttorder,pct_pass,dataname,char(dirname{1}),opt_band);
else
    [~,~,bands,~,~,A,P,F,cycle_bounds]=filter_fft_bank(data,sampling_freq,min_cycles,10,[1/signal_length sampling_freq/2],spacing,buttorder,pct_pass,dataname,char(dirname{1}));
end
...
[MI_p_vals,MI_p_thresh,MI_z_scores,MI_z_pvals,MI_z_thresh,MI_lz_scores,MI_lz_pvals,MI_lz_thresh]=inv_entropy_distn_w_thresh(noshufs,threshold,nobins,A,P,cycle_bounds,cycle_bounds,MI,dataname,bands,bands);
...
Binned_MI_lzt=emd_inv_entropy_plot_bMI(MI_lz_thresh,F,x_lims,x_bins,y_lims,y_bins,'linear',1,units,[dataname,'_lzt']);

MI_npv = max(0,MI_z_pvals-threshold);
Binned_NPV=emd_inv_entropy_plot_bMI(MI_npv,F,x_lims,x_bins,y_lims,y_bins,'linear',1,units,[dataname,'_npv']);

MI_lpv = max(0,MI_lz_pvals-threshold);
Binned_LPV=emd_inv_entropy_plot_bMI(MI_lpv,F,x_lims,x_bins,y_lims,y_bins,'linear',1,units,[dataname,'_lpv']);
