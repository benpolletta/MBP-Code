function avp_norm_avg=collect_avp_beta(nobins,noamps,nophases,amps,phases,units)
...
%         avp_norm_avg(1:2*r,1:c,j)=nansum(temp,3);
        ...
        avp_norm_avg(:,:,j)=avp_norm_avg(:,:,j)+avp_norm/dirnum;
...
max_amp=max(max(max(avp_norm_avg(:,1:end-2,1:end-2,:))));
min_amp=min(min(min(avp_norm_avg(:,1:end-2,1:end-2,:))));
