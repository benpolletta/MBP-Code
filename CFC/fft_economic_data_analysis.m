function fft_economic_data_analysis(orders)
...
band_centers_lo=.005*[1:10];
no_bands_lo=length(band_centers_lo);
bands_lo=[band_centers_lo'-.002 band_centers_lo' band_centers_lo'+.002];
band_centers_hi=.02*[1:24];
no_bands_hi=length(band_centers_hi);
bands_hi=[band_centers_hi'-.01 band_centers_hi' band_centers_hi'+.01];
...
%     max_bands=zeros(50,5,2,listnum);
    ...
    for j=1:listnum
        ...
        for k=1:filenum
            ...
            [bands_lo,bands_hi,MI(:,:,k,1),MI(:,:,k,2),MI(:,:,k,3),MI(:,:,k,4),MI(:,:,k,5)]=CFC_April_fft_beta(char(filenames(k)),600,20,100,.05,.05,.005*[1 10],10,.02*[1 24],24,order,pct_pass,'Hz',dirnames,bands_lo,bands_hi);
