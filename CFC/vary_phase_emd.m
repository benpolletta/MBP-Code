function [times,max_bands,H]=vary_phase_emd
...
for j=1:10
    ...
    percent_bw_labels{j}=[num2str(6-6*j*.05),' - ',num2str(6+6*j*.05)];

    filenames=textread(['6pm',num2str(6*j*.05),'.hht.list'],'%s%*[^\n]');
    ...
    fid=fopen(['avg_pMI_vary_phase_',num2str(6),'pm',num2str(6*j*.05),'.txt'],'w')
    ...
    fid1=fopen(['max_pMI_bands_vary_phase_',num2str(6),'pm',num2str(6*j*.05),'.txt'],'w')
    fprintf(fid1,'%f\t%f\n',reshape(max_pMI_bands(:,j,:),50,2)')

    fid2=fopen(['max_bands_hi_vary_phase_',num2str(6),'pm',num2str(6*j*.05),'.txt'],'w')
    fprintf(fid2,'%s\t%s\t%s\n',char(graph_labels{1}),char(graph_labels{2}),char(graph_labels{3}))
    fprintf(fid2,'%f\t%f\t%f\n',reshape(max_bands(:,j,:,1),50,3)')

    fid3=fopen(['max_bands_lo_vary_phase_',num2str(6),'pm',num2str(6*j*.05),'.txt'],'w')
    fprintf(fid3,'%s\t%s\t%s\n',char(graph_labels{1}),char(graph_labels{2}),char(graph_labels{3}))
    fprintf(fid3,'%f\t%f\t%f\n',reshape(max_bands(:,j,:,2),50,3)')

    fid4=fopen(['emd_times_vary_phase_',num2str(6),'pm',num2str(6*j*.05),'.txt'],'w')
    fprintf(fid4,'%f\n',times(:,j))
    ...
    figure()
    colorplot(avgpMI(:,:,:))
    title(['Mean Percent Modulation Index, Slow Osc. from ',num2str(6-6*j*.05),'-',num2str(6+6*j*.05),'Hz'],'FontSize',30)
    axis xy
    ...
    saveas(gcf,['avg_pMI_6pm',num2str(j*6*.05),'.fig'])

    figure()
    hist(max_pMI_bands(:,j,1),bands_hi)
    title(['Histogram of Max. Amplitude Bin for ',graph_titles{4},', Low Freq. Osc. from ',num2str(6-6*j*.05),'-',num2str(6+6*j*.05),'Hz'])
    set(gca,'XTick',bands_hi,'XTickLabel',A_labels)
    xlabel('Center Frequency (Hz)')
    saveas(gcf,['hist_',graph_labels{4},'_hi_bin_vary_phase_',num2str(6),'pm',num2str(6*j*.05),'.fig'])

    figure()
    hist(max_pMI_bands(:,j,2),bands_lo)
    title(['Histogram of Max. Phase Bin for ',graph_titles{4},', Low Freq. Osc. from ',num2str(6-6*j*.05),'-',num2str(6+6*j*.05),'Hz'])
    set(gca,'XTick',bands_lo,'XTickLabel',P_labels)
    xlabel('Center Frequency (Hz)')
    saveas(gcf,['hist_',graph_labels{4},'_lo_bin_vary_phase_',num2str(6),'pm',num2str(6*j*.05),'.fig'])
end
...
%     xlabel('Frequency Range, Slow Oscillation')
    ...
    %         saveas(gcf,['mean_',graph_labels{l},'_bin_hi_vary_phase_',num2str(6),'.fig'])
...
%     title(['Mean\pm S.D. of Phase Frequency for Max. ',graph_titles{l}])
    ...
    %         saveas(gcf,['mean_',graph_labels{l},'_bin_lo_vary_phase_',num2str(6),'.fig'])
    saveas(gcf,['mean_',graph_labels{l},'_bin_vary_phase.fig'])

end

H(:,:,1)=hist(max_pMI_bands(:,:,1),bands_hi);

fid5=fopen('vary_phase_hists_hi_pMI.txt','w');
fprintf(fid5,'%f\t%f\t%f\t%f\t%f\n',H(:,:,1)');

H(:,:,2)=hist(max_pMI_bands(:,:,2),bands_lo);

fid6=fopen('vary_phase_hists_lo_pMI.txt','w');
...
% xlabel('Frequency Range, Slow Oscillation')
%     saveas(gcf,['hist_',graph_labels{4},'_hi_bin_vary_phase_',num2str(6),'.fig'])
...
xlabel('Frequency Range, Slow Oscillation','FontSize',24)
%     saveas(gcf,['hist_',graph_labels{4},'_lo_bin_vary_phase_',num2str(6),'.fig'])
saveas(gcf,['hist_',graph_labels{4},'_bin_vary_phase.fig'])
...
xlabel('Frequency Range, Slow Oscillation')
saveas(gcf,['emd_vary_phase_times_boxplot.fig'])