function [bands,MI,MI_p_vals,MI_z_scores,MI_log_z_scores,Binned_MI_pt,Binned_MI_zt,Binned_MI_lzt,Binned_NPV,Binned_LPV]=CFC_April_fbank(datafile,sampling_freq,min_cycles,nobins,noshufs,threshold,x_lims,x_bins,y_lims,y_bins,buttorder,pct_pass,spacing,units,outfilename,extradirname,opt_band)
...
[MI_p_vals,MI_z_scores,MI_norm_pvals,MI_log_z_scores,MI_lognorm_pvals]=inv_entropy_distn(noshufs,nobins,A,P,cycle_bounds,cycle_bounds,MI,dataname,bands,bands);

[MI_p_thresh,MI_z_thresh,MI_log_z_thresh]=inv_entropy_colorplot(triu(MI,1),triu(MI_p_vals,1),triu(MI_z_scores,1),triu(MI_log_z_scores,1),threshold,dataname,bands,bands,units);
...
Binned_MI_lzt=emd_inv_entropy_plot_bMI(MI_log_z_thresh,F,x_lims,x_bins,y_lims,y_bins,'linear',units,[dataname,'_lzt']);
Binned_NPV=emd_inv_entropy_plot_bMI(MI_norm_pvals,F,x_lims,x_bins,y_lims,y_bins,'linear',units,[dataname,'_npv']);
Binned_LPV=emd_inv_entropy_plot_bMI(MI_lognorm_pvals,F,x_lims,x_bins,y_lims,y_bins,'linear',units,[dataname,'_lpv']);
