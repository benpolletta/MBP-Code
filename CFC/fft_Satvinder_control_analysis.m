function fft_Satvinder_control_analysis(orders)
...
band_centers_hi=20:5:180;
bands_hi=[band_centers_hi'-2.5 band_centers_hi' band_centers_hi'+2.5];
no_bands_hi=length(band_centers_hi);

band_centers_lo=4:.25:12;
bands_lo=[band_centers_lo'-2.5 band_centers_lo' band_centers_lo'+2.5];
...
%             tic;
            ...
            [~,~,temp_MI(:,:,:,1),temp_MI(:,:,:,2),temp_MI(:,:,:,3),temp_MI(:,:,:,4),temp_MI(:,:,:,5)]=CFC_April_fft_beta(char(filenames(k)),600,20,100,.05,.05,[4 12],no_bands_lo,[20 180],no_bands_hi,order,pct_pass,'Hz',dirnames);
