function fft_sim_analysis(orders,challenge_list,challenge_name,challenge_descriptor,challenge_params,challenge_labels)
...
for m=1:length(orders)
    ...
    for j=1:listnum
        ...
        for k=1:filenum
            ...
            [bands_lo,bands_hi,MI(:,:,k,1),MI(:,:,k,2),MI(:,:,k,3),MI(:,:,k,4),MI(:,:,k,5)]=CFC_April_fft(char(filenames(k)),600,20,100,.05,.05,[3 9],15,[20 110],15,order,pct_pass,'Hz',dirnames);
        ...
        for m=1:50
            for n=1:5
                ...
                if maxmi~=0
                    ...
                    max_bands(m,n,1,j)=nan;
                    max_bands(m,n,2,j)=nan;
                end
            end
        end
