function [f,data_hat,bands,signals,H,A,P,F,cycle_bounds]=filter_fft_fancy(data,varargin)
...
%     save figures.
% 'spacing', 'linear' or 'log' or 'sqrt' or 'dyadic' or a positive real number: 
%     Determines how a range of frequencies is divided into bins. Default is linear.
...
% 'bank_design', 'parallel' or 'serial': Determines whether bandpass
%   filters are applied to the data in parallel, or highpass filters are
%   applied serially to create a filter bank.
...
for i=1:floor(length(varargin)/2)
    ...
    elseif strcmp(varargin(2*i-1),'bank_design')==1
        bank_design=char(varargin(2*i)); 
    end
end
...
if strcmp(bank_design,'parallel')==1
    ...
    for j=1:nobands
        ...
        if buttorder==-1
            filter=butterworth(buttorder,signal_length,sampling_freq,plo,phi,0);
        else
            if buttorder==0
            ...
            [z,p,k]=butter(n/2,Wn); [sos,g]=zp2sos(z,p,k); h=dfilt.df2sos(sos,g);
...
elseif strcmp(bank_design,'serial')==1
