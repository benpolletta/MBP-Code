...
%specified on Line 34. Filtering is performed via a 3rd-order, symmetrical 
...
ampsum = zeros(1,20);   %sum of amplitudes at a given phase bin
ampoccur = zeros(1,20); %number of amplitudes at a given phase bin

t = 0:delta:(nsample-1)*delta;  %time steps are size 1/fs

x = load('..\..\Test_hht_dat\Signals\dow_jones_industrial_average_raw.dat');
x(nsample+1:length(x)) = [];    %ignoring last few extra data points

windowt = 1000; %width of window to step along time series
nwindow = nsample/windowt;  %number of windows

%%% choose frequency ranges %%%
for M = 1:10
    for N = 1:24
        phase = 0.005*M;    % cycle length from 20 (.05) to 200 (.005)
        Amp = 0.02*N;       % cycle length from 50/24=2.0833 (.48) to 50 (.02)
        flo(1) = phase-0.002;   % .003 to .007, .008 to .012, .013 to .017, ..., .048 to .052
        fhi(1) = phase+0.002;   % from one 250th up to one tenth of the Nyquist frequency.
        flo(2) = Amp-0.01;      % .01 to .03, .03 to .05, .05 to .07, ..., .47 to .49
        fhi(2) = Amp+0.01;      % up to the Nyquist frequency.
        
        %%% set up butterworth filtering weights %%%
        period = windowt/fs;    %length of window
        hzpbin = 1/period;      %dunno
        for j = 1:2
            for i = 1:windowt/2+1
        ...
        legend('low-frequency extraction','high-frequency extraction')
        xlabel('frequency')
        ylabel('Power');
        ...
        for k = 1:nwindow
            ...
            fftx = fft(xk);
            for i = 1:windowt
                fftx_lf(i) = fftx(i) * sqrt(factor_lo(1,i) * factor_hi(1,i));    %obtaining low freq. signal, freq. domain
                fftx_hf(i) = fftx(i) * sqrt(factor_lo(2,i) * factor_hi(2,i));    %obtaining hi freq. signal, freq. domain
            end
            lfx([1:windowt]+(k-1)*windowt) = ifft(fftx_lf); %obtaining low freq. signal, time domain
            hfx([1:windowt]+(k-1)*windowt) = ifft(fftx_hf); %obtaining high freq. signal, time domain
        end
        plot(t,lfx,'r');
        plot(t,hfx,'g');
        ...
        hlfx = hilbert(lfx);
        hhfx = hilbert(hfx);
        phaself = angle(hlfx);  %getting phases of high-freq. signal
        amphf = abs(hhfx);      %getting amplitude of low-freq. signal
        figure(3);
        plot(t,phaself);
        ...
        for i = 1:nsample
            for k = 1:20
                if ((k-11)*0.1*pi < phaself(i) && phaself(i) < (k-10)*0.1*pi) %bins amplitudes by phase, w/ 20 bins from k=-pi to k=pi
                    ampsum(k) = ampsum(k) + amphf(i);  %there is a better way to do this :) at least i think it's better
