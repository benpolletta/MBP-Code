...
for j=1:no_challenges
    ...
    for k=1:filenum
        ...
        [bands_lo,bands_hi,MI(:,:,k)]=CFC_Jan_wav(char(filenames(k)),400,20,linspace(4.25,11.75,16)',linspace(25,175,16)','Hz',dirnames);
...
%     close(gcf)
    ...
    %         figure()
    [h,b]=hist(max_bands(:,2),bands_lo(:,2));
    H_MI(:,j,2)=h/filenum;
    subplot(2,1,2)
    bar(b,h)
...
if no_challenges>1
    ...
    fid5=fopen([challenge_list_name,'_hists_hi_MI.txt'],'w');
    fprintf(fid5,H_format,[challenge_params; H_MI(:,:,1)]');
    
    fid6=fopen([challenge_list_name,'_hists_lo_MI.txt'],'w');
    ...
    colorplot(H_MI(:,:,1))
    axis xy
    title('Histogram of Max. Bin for MI','FontSize',30)
    ...
    saveas(gcf,['hist_MI_',challenge_list_name,'.fig'])
...
%     end
%     saveas(gcf,['hist_MI_',challenge_list_name,'_minimal'],'pdf')
%     close(gcf)
