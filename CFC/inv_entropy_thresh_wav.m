function inv_entropy_thresh_wav(filedir,noamps,nophases,x_lims,x_bins,y_lims,y_bins,units)
...
for i=1:filenum
    ...
    F_lo_filename=[filedir,'/HAF_Data_Plots/',filename,'_lo/',filename,'_lo_freqs.txt'];
    F_hi_filename=[filedir,'/HAF_Data_Plots/',filename,'_hi/',filename,'_hi_freqs.txt'];
    ...
    F_lo=load(F_lo_filename);
    F_hi=load(F_hi_filename);
...
%     inv_entropy_bMI_all(mit,F_lo,F_hi,50,'linear',units,[filename,'_thresh']);
    bMI_thresh(:,:,i)=inv_entropy_plot_bMI(mit,F_lo,F_hi,x_lims,x_bins,y_lims,y_bins,'linear',1,units,[filename,'_thresh']);
...
fid=fopen([listname(1:end-5),'_avg_MI.txt'],'w');
...
fid=fopen([listname(1:end-5),'_avg_bMI.txt'],'w');
fprintf(fid,format,avg_MI_thresh');
fclose(fid);
...
axis xy
set(gca,'XTick',[1.5:x_bins+1.5],'YTick',[1.5:y_bins+1.5],'XTickLabel',bands_lo,'YTickLabel',bands_hi,'FontSize',16)
xlabel(['Phase-Modulating Freq. (',char(units),')'],'FontSize',24)
ylabel(['Amplitude-Modulated Freq. (',char(units),')'],'FontSize',24)

cd (Cutoff_dir)

saveas(1,[listname(1:end-5),'_avg_MI.fig'])
saveas(2,[listname(1:end-5),'_avg_bMI.fig'])
