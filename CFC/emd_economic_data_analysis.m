function emd_economic_data_analysis

challenge_list='Dow_Jones_hht_list.list';
listnames=textread(challenge_list,'%s%*[^\n]');
no_challenges=length(listnames);

challenge_name='Dow_Jones';
challenge_descriptor='Dow Jones Industrial Average';
challenge_params=[];
challenge_labels={''};
...
for j=1:no_challenges
    ...
    max_bands=zeros(filenum,2,no_challenges,3);
    max_nMI_bands=zeros(filenum,2,no_challenges,3);
    nMI=zeros(no_bands_hi,no_bands_lo,filenum,2);
...
%         filename=[filename,'_low_hi.txt'];
        [bands,MI(:,:,1),MI(:,:,2),MI(:,:,3),nMI(:,:,k,1),nMI(:,:,k,2),nMI(:,:,k,3)]=CFC_April_emd_beta(filename,1,5,20,100,.05,.05,[.005 .05],10,[.02 .48],24,'linear','Hz',dirnames);
...
%         [bands,MI(:,:,1),MI(:,:,2),MI(:,:,3),nMI(:,:,k,1),nMI(:,:,k,2)]=CFC_April(char(filenames(k)),600,5,20,100,.05,.05,[3 9],15,[20 110],15,'linear','Hz',dirnames);
    ...
    for l=1:3
        ...
        fid=fopen(['avg_',graph_labels{3+l},'_',challenge_name,'.txt'],'w')
        ...
        fid1=fopen(['max_',graph_labels{3+l},'_bands_',challenge_name,'.txt'],'w')
        fprintf(fid1,'%f\t%f\n',max_nMI_bands(:,:,j,l)')

    end

    fid2=fopen(['max_bands_hi_',challenge_name,'.txt'],'w')
    fprintf(fid2,'%s\t%s\t%s\n',char(graph_labels{1}),char(graph_labels{2}),char(graph_labels{3}))
    fprintf(fid2,'%f\t%f\t%f\n',reshape(max_bands(:,1,j,:),filenum,3)')

    fid3=fopen(['max_bands_lo_',challenge_name,'.txt'],'w')
    fprintf(fid3,'%s\t%s\t%s\n',char(graph_labels{1}),char(graph_labels{2}),char(graph_labels{3}))
    fprintf(fid3,'%f\t%f\t%f\n',reshape(max_bands(:,2,j,:),filenum,3)')

    fid4=fopen(['emd_times_',challenge_name,'.txt'],'w')
    fprintf(fid4,'%f\n',times(:,j))
    ...
    for l=1:3
        ...
        axis xy
        set(gca,'XTick',[1.5:10.5],'YTick',[1.5:24.5],'XTickLabel',P_labels,'YTickLabel',A_labels)
        xlabel('Phase-Modulating Frequency (Hz)')
        ylabel('Amplitude-Modulated Frequency (Hz)')
        saveas(gcf,['avg_',graph_labels{3+l},'_',challenge_name,'.fig'])

        figure()
        colormap('gray')
        colorplot(avgnMI(:,:,:,l))
        axis xy
        set(gca,'XTick',[1.5 4.5 7.5 10.5],'YTick',[0:11:24]+1.5,'XTickLabel',P_labels([1:3:10]),'YTickLabel',A_labels([1:11:24]),'FontSize',16)
        saveas(gcf,['avg_',graph_labels{3+l},'_',challenge_name,'_minimal.fig'])
        ...
        saveas(gcf,['hist_',graph_labels{3+l},'_hi_bin_',challenge_name,'.fig'])
        ...
        xlabel('Center Frequency (Hz)')
        saveas(gcf,['hist_',graph_labels{3+l},'_lo_bin_',challenge_name,'.fig'])
