function [f,data_hat,bands,signals,H,A,P,F,cycle_bounds]=filter_fft_reflect(data,varargin)
...
% Reflecting ends of data to correct edge artifacts.
...
% signals=zeros(signal_length,nobands); H=zeros(signal_length,nobands);
% A=zeros(signal_length,nobands); P=zeros(signal_length,nobands);
% F=zeros(signal_length,nobands); cycle_bounds=[];
...
signals=signals(halflength+(1:datalength),:);
