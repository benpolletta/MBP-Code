...
for i=1:noreps
    ...
    no_kinks=round(trend_level*no_low_cycles);
    slope_max=2*sampling_freq*trend_level/low_cycle_length;

    kinks=sort(max(round(rand(1,no_kinks)*simlength),1));
    kinks=[kinks simlength];
    slopes=2*rand(1,no_kinks+1)*slope_max-slope_max;
    int=4*rand-2;
