function [bands,MI,MI_p_vals,MI_z_scores,Percent_MI,Binned_MI]=CFC_April_fbank(datafile,sampling_freq,min_cycles,nobins,noshufs,p_threshold,z_threshold,x_lims,x_bins,y_lims,y_bins,buttorder,pct_pass,spacing,units,outfilename,extradirname)
...
if max(max(MI_z_thresh))>0
    ...
    emd_inv_entropy_pMI_all(MI_z_thresh,F,50,'linear',units,dataname);
    Binned_MI=emd_inv_entropy_plot_bMI(MI_z_thresh,F,x_lims,x_bins,y_lims,y_bins,'linear',units,dataname);
    Percent_MI=emd_inv_entropy_plot_pMI(MI_z_thresh,F,x_lims,x_bins,y_lims,y_bins,'linear',units,dataname);
elseif max(max(MI_p_thresh))>0
    emd_inv_entropy_plot_scatter(MI_p_thresh,F,units,dataname);
    emd_inv_entropy_pMI_all(MI_p_thresh,F,50,'linear',units,dataname);
%     Percent_MI=emd_inv_entropy_plot_pMI(MI_p_thresh,F,x_lims,x_bins,y_lims,y_bins,'linear',units,dataname);
    Binned_MI=zeros(x_bins,y_bins);
    Percent_MI=zeros(x_bins,y_bins);
else
