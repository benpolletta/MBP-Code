function [times,max_bands]=vary_amp_fbank
graph_titles{1}='MI';
...
for m=1:2
    ...
    for j=1:10
        ...
        bw_labels{j}=[num2str(65*(1-j*.05)),' - ',num2str(65*(1+j*.05))];

        filenames=textread(['65pm',num2str(j*.05*65),'.list'],'%s%*[^\n]');
        ...
        fid=fopen(['avg_pMI_vary_amp_65pm',num2str(j*.05*65),'.txt'],'w')
        ...
        fid1=fopen(['max_pMI_bands_vary_amp_65pm',num2str(j*.05*65),'.txt'],'w')
        fprintf(fid1,'%f\t%f\n',max_pMI_bands(:,:,j)')

        fid2=fopen(['max_bands_hi_vary_amp_65pm',num2str(j*.05*65),'.txt'],'w')
        fprintf(fid2,'%s\t%s\t%s\n',char(graph_labels{1}),char(graph_labels{2}),char(graph_labels{3}))
        fprintf(fid2,'%f\t%f\t%f\n',reshape(max_bands(:,j,:,1),50,3)')

        fid3=fopen(['max_bands_lo_vary_amp_65pm',num2str(j*.05*65),'.txt'],'w')
        fprintf(fid3,'%s\t%s\t%s\n',char(graph_labels{1}),char(graph_labels{2}),char(graph_labels{3}))
        fprintf(fid3,'%f\t%f\t%f\n',reshape(max_bands(:,j,:,2),50,3)')

        fid4=fopen(['emd_times_vary_amp_65pm',num2str(j*.05*65),'.txt'],'w')
        fprintf(fid4,'%f\n',times(:,j))

        figure()
        colorplot(avgpMI(:,:,:))
        title(['Mean Percent MI, Fast Osc. From ',num2str(65*(1-j*.05)),' - ',num2str(65*(1+j*.05)),' Hz'],'FontSize',30)
        axis xy
        ...
        saveas(gcf,['avg_pMI_vary_amp_65pm',num2str(j*.05*65),'.fig'])

        figure()
        subplot(2,1,1)
        hist(max_pMI_bands(:,1,j),bands_hi)
        title(['Histogram of Max. Bin for ',graph_titles{4},', Fast Osc. From ',num2str(65*(1-j*.05)),' - ',num2str(65*(1+j*.05)),' Hz'],'FontSize',30)
        xlabel('Amp. Bin Center Freq. (Hz)')

        subplot(2,1,2)
        hist(max_pMI_bands(:,2,j),bands_lo)
        xlabel('Phase Bin Center Freq. (Hz)')
        saveas(gcf,['hist_',graph_labels{4},'_bin_vary_amp_65pm',num2str(j*.05*65),'.fig'])
    ...
    for l=1:3
        ...
        xlabel('Frequency Range, Fast Osc.','Fontsize',24)
        saveas(gcf,['boxplot_',graph_labels{l},'_vary_amp_65pm',num2str(j*.05*65),'.fig'])
    ...
    xlabel('Frequency Range, Fast Osc.')
    saveas(gcf,['vary_amp_fbank_times_boxplot.fig'])
