function [times,max_bands]=vary_phase_fbank
graph_titles{1}='MI';
...
for m=1:2
    ...
    for j=1:10
        ...
        bw_labels{j}=[num2str(6*(1-j*.05)),' - ',num2str(6*(1+j*.05))];

        filenames=textread(['6pm',num2str(j*.05*6),'.list'],'%s%*[^\n]');
        ...
        fid=fopen(['avg_pMI_vary_phase_6pm',num2str(j*6*.05),'.txt'],'w')
        ...
        fid1=fopen(['max_pMI_bands_vary_phase_6pm',num2str(j*6*.05),'.txt'],'w')
        fprintf(fid1,'%f\t%f\n',max_pMI_bands(:,:,j)')

        fid2=fopen(['max_bands_hi_vary_phase_6pm',num2str(j*6*.05),'.txt'],'w')
        fprintf(fid2,'%s\t%s\t%s\n',char(graph_labels{1}),char(graph_labels{2}),char(graph_labels{3}))
        fprintf(fid2,'%f\t%f\t%f\n',reshape(max_bands(:,j,:,1),50,3)')

        fid3=fopen(['max_bands_lo_vary_phase_6pm',num2str(j*6*.05),'.txt'],'w')
        fprintf(fid3,'%s\t%s\t%s\n',char(graph_labels{1}),char(graph_labels{2}),char(graph_labels{3}))
        fprintf(fid3,'%f\t%f\t%f\n',reshape(max_bands(:,j,:,2),50,3)')

        fid4=fopen(['emd_times_vary_phase_6pm',num2str(j*6*.05),'.txt'],'w')
        fprintf(fid4,'%f\n',times(:,j))

        figure()
        colorplot(avgpMI(:,:,:))
        title(['Mean Percent MI, Slow Osc. From ',num2str(6*(1-j*.05)),' - ',num2str(6*(1+j*.05)),' Hz'],'FontSize',30)
        axis xy
        ...
        saveas(gcf,['avg_pMI_vary_phase_6pm',num2str(j*6*.05),'.fig'])

        figure()
        subplot(2,1,1)
        hist(max_pMI_bands(:,1,j),bands_hi)
        title(['Histogram of Max. Bin for ',graph_titles{4},', Slow Osc. From ',num2str(6*(1-j*.05)),' - ',num2str(6*(1+j*.05)),' Hz'],'FontSize',30)
        xlabel('Amp. Bin Center Freq. (Hz)')

        subplot(2,1,2)
        hist(max_pMI_bands(:,2,j),bands_lo)
        xlabel('Phase Bin Center Freq. (Hz)')
        saveas(gcf,['hist_',graph_labels{4},'_bin_vary_phase_6pm',num2str(j*6*.05),'.fig'])
    ...
    for l=1:3
        ...
        saveas(gcf,['boxplot_',graph_labels{l},'_vary_phase_6pm',num2str(j*6*.05),'.fig'])
    ...
    set(gca,'YTick',1.5:16.5,'YTickLabel',A_labels,'XTick',1.5:11.5,'XTickLabel',bw_labels,'FontSize',16)
    ylabel({'Amp. Bin';'Center Freq. (Hz)'},'FontSize',24)

    subplot(2,1,2)
    colorplot(H(:,:,2))
    axis xy
    set(gca,'YTick',1.5:16.5,'YTickLabel',P_labels,'XTick',1.5:11.5,'XTickLabel',bw_labels,'FontSize',16)
    ...
    saveas(gcf,['vary_phase_fbank_times_boxplot.fig'])
