function fft_sim_analysis_reflect
...
challenge_name='noise_reflected';
...
for m=1:length(orders)
    ...
    graph_titles{4}='p-Thresholded MI';
    graph_titles{5}='z-Thresholded MI';
...
%             tic;
            ...
            [bands_lo,bands_hi,MI(:,:,k,1),MI(:,:,k,2),MI(:,:,k,3),MI(:,:,k,4),MI(:,:,k,5)]=CFC_April_fft_reflect(char(filenames(k)),600,20,100,.05,.05,[3 9],15,[20 110],15,order,pct_pass,'Hz',dirnames);
