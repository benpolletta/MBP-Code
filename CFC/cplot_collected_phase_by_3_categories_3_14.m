function cplot_collected_phase_by_3_categories_3_14(Title,name,freqs,stops,c_order,cat3_labels,cat1_labels,cat2_labels,cat3_vec,cat1_vec,cat2_vec,spec)
...
stat_labels={'mean','dispersion'};
long_stat_labels={'Mean','Dispersion'};
    ...
    for c1=1:no_cats1
        ...
        for c2=1:no_cats2
            ...
            if ~isempty(spec_cat2) && size(spec_cat2,1)>=5
                ...
                spec_stats(:,c1,c2,1)=angle(nanmean(exp(sqrt(-1)*spec_cat2)))';
                
                spec_stats(:,c1,c2,2)=abs(nanmean(exp(sqrt(-1)*spec_cat2)))'/size(spec_cat2,2);
                
            else
                
                spec_stats(:,c1,c2,1:2)=nan;
...
%                 band_freqs{b}=freqs(band_indices{b});
%                 
...
%                 end
                ...
                figure(), imagesc(reshape(spec_stats(:,c1,:,m),no_freqs,no_cats2))
                
                figs(m,c1,c3)=gcf;
                
                handl=figure_replotter(gcf,1,1,5,18,cat2_labels,freqs,{[long_stat_labels{m},' ',long_cat1_labels{c1},' ',long_cat3_labels{c3}]});
                
                saveas(handl,[name,'_',cat1,'_',cat3,'_',stat_labels{m},'.fig']);
...
for c1=1:no_cats1
    ...
    for m=1:no_stats
        ...
        c1_figs=reshape(figs(m,c1,:),no_cats3,1);
        
        figure_replotter(c1_figs,1,no_cats3,4,7,cat2_labels,freqs,long_cat3_labels)
        
        saveas(gcf,[name,'_',cat1_labels{c1},'_',stat_labels{m},'_fr.fig'])
        
%         saveas(no_cats1+c1,[name,'_',cat1_labels{c1},'_',stat_labels{m},'_line.fig'])
...
for c3=1:no_cats3
    ...
    for m=1:no_stats
        ...
        c3_figs=reshape(figs(m,:,c3),no_cats1,1);
        
        figure_replotter(c3_figs,1,no_cats1,4,7,cat2_labels,freqs,long_cat1_labels)
        
        saveas(gcf,[name,'_',cat3_labels{c3},'_',stat_labels{m},'_fr.fig'])
        
%         saveas(no_cats1+c1,[name,'_',cat3_labels{c3},'_',stat_labels{m},'_line.fig'])
