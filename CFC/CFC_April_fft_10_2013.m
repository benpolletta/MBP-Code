function [bands_lo,bands_hi,MI,MI_p_vals,MI_z_scores,MI_p_thresh,MI_z_thresh]=CFC_April_fft_10_2013(datafile,sampling_freq,nobins,noshufs,p_threshold,z_threshold,range_lo,nobands_lo,range_hi,nobands_hi,buttorder,pct_pass,units,dirname,opt_band_lo,opt_band_hi)
...
if isempty(opt_band_lo)
    [~,~,bands_lo,~,~,~,P_lo,~,cycle_bounds_lo]=filter_fft_new(data,'fs',sampling_freq,'spacing',spacing,'filename',[dataname,'_lo'],'bandrange',range_lo,'nobands',nobands_lo,'buttorder',buttorder,'pct_pass',pct_pass,'dirname',char(dirname{1}));
else
    [~,~,bands_lo,~,~,~,P_lo,~,cycle_bounds_lo]=filter_fft_new(data,'fs',sampling_freq,'filename',[dataname,'_lo'],'bands',opt_band_lo,'buttorder',buttorder,'dirname',char(dirname{1}));
end

if isempty(opt_band_hi)
    [~,~,bands_hi,~,~,A_hi,~,~,cycle_bounds_hi]=filter_fft_new(data,'fs',sampling_freq,'spacing',spacing,'filename',[dataname,'_hi'],'bandrange',range_hi,'nobands',nobands_hi,'buttorder',buttorder,'pct_pass',pct_pass,'dirname',char(dirname{1}));
else
    [~,~,bands_hi,~,~,A_hi,~,~,cycle_bounds_hi]=filter_fft_new(data,'fs',sampling_freq,'filename',[dataname,'_hi'],'bands',opt_band_hi,'buttorder',buttorder,'dirname',char(dirname{1}));
end
