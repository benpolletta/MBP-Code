function [MI_p_vals,MI_p_thresh,MI_z_scores,MI_norm_pvals,MI_z_thresh,MI_log_z_scores,MI_lognorm_pvals,MI_lz_thresh]=inv_entropy_distn_w_thresh(noshufs,threshold,nobins,A,P,cycle_bounds_A,cycle_bounds_P,MI,filename,A_bands,P_bands)
...
%         mkdir ([filename,'_inv_entropy'])
        shuf_name=[filename,'_',num2str(noshufs),'_shufs_p_',num2str(threshold),'.txt'];
    else
    ...
    for i=1:noshufs
        ...
        [~,M_shuf,~]=amp_v_phase_no_save(nobins,A_shuf,P_shuf);
        %     Mvals(i,:,:,:)=M_shuf;
        %     for l=1:nophases
        %         for m=1:noamps
