function [MI_all]=wavelet_mouse_eeg_collect_Bernat_summed(subject,channel,amp_lims,phase_lims)

% amp_limits and phase_limits are row vectors containing limits on
% amplitude and phase frequencies, respectively.
% Run from parent directory (Bernat_NMDAR_antagonists).
...
%     epochs_per_period=end_epochs-start_epochs+1;
    ...
    all_filename=['ALL_',epoch_list(1:end-5)];
    
    fid=fopen([all_dirname,'/',all_filename,'_',freq_label,'.txt'],'w');
...
%             if epoch_state<no_states
            ...
            for i=1:no_measures
                ...
                MI_sum=sum(sum(MI(amp_indices,phase_indices)));
    ...
    save([all_dirname,'/',all_filename,'_',freq_label,'.mat'],'MI_all')
...
%         plot([t(start_epochs(2)-start_epochs(1)+1) t(start_epochs(2)-start_epochs(1)+1)],ylim,'k','LineWidth',2)
        ...
        saveas(gcf,[all_dirname,'/',all_filename,'_',measures{m},'_',freq_label,'.fig'])
...
for m=1:no_measures
    ...
    saveas(gcf,[all_dirname,'/',subject,'_chan',num2str(channel),'_',measures{m},'_',freq_label,'.pdf'])

    subplot(4,1,1)
    
    legend(state_labels)
    
    saveas(gcf,[all_dirname,'/',subject,'_chan',num2str(channel),'_',measures{m},'_',freq_label,'.fig'])
