function All_cross_PAC_multisummed(channel1_label, channel2_label, norms, amp_limits, phase_limits)
...
name = sprintf('ALL_%s_A_by_%s_P_PAC', channel1_label, channel2_label);

for n = 1:length(norms)
    
    MI = load([name, '/', name, norms{n}, '.txt']);
    ...
    for al=1:no_als
        ...
        for pl=1:no_pls
            ...
            band_labels{r} = [num2str(amp_limits(al,1)),'to',num2str(amp_limits(al,2)),...
                'by',num2str(phase_limits(pl,1)),'to',num2str(phase_limits(pl,2))];
    ...
    save([name, '/', name, norms{n}, '_summed.mat'], 'band_labels', 'summed_MI')
