...
no_pre=2; no_post=8;
[BP_hr_labels, ~, ~] = make_period_labels(no_pre, no_post, '6mins');
no_BP_hr_periods = length(BP_hr_labels);
short_BP_hr_labels = -117:6:477; short_BP_hr_labels(short_BP_hr_labels == 0) = [];
...
for n=1:no_norms
    ...
    for c=1:no_channels
        ...
        suffix = ['hrMI', norms{n}, '_6min_BP_stats'];
    ...
    for s = 1:no_stats
        ...
        for b = 1:no_bands_plotted
            ...
            for d = 2:(no_drugs - 1)
                ...
                set(gca, 'XTick', 1:tick_spacing:no_BP_hr_periods, 'XTickLabel', round(short_BP_hr_labels(1:tick_spacing:end)/60), 'FontSize', 16)
        ...
        save_as_pdf(gcf,['ALL_summed_MI',norms{n},'_multichannel_MK801_NVP_6min_', stats{s}, cplot_norm]) %, 'orientation', 'portrait')
