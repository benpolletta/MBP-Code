function wavelet_mouse_eeg_compute_power_Bernat(subject,channel)
...
spec_format=make_format(no_f_bins+2,'f');
...
%     
   ...
   for j=1:total_epochs
        ...
        data_hat=pmtm(data,[],sampling_freq);
    ...
    fprintf(fid_vec(1),spec_format,[(1:total_epochs)' drug_epoch_states spec_all]');
    fprintf(fid_vec(2),BP_format,[(1:total_epochs)' drug_epoch_states BP_all]');
    ...
    spec_all_std_inv=diag(1./nanstd(spec_all));
    spec_all_norm=(spec_all-spec_all_mean)*spec_all_std_inv;
    
%     BP_all_mean=ones(size(BP_all))*diag(nanmean(BP_all));
%     BP_all_std_inv=diag(1./nanstd(BP_all));
%     BP_all_norm=(BP_all-BP_all_mean)*BP_all_std_inv;
...
%     end
    ...
    set(gca,'YTick',y_ticks,'XTickLabel',f_bin_centers(y_ticks))
...
%     set(gca,'XTick',x_ticks,'XTickLabel',t(x_ticks))
%     y_ticks=1:floor(no_f_bins/10):no_f_bins;
%     set(gca,'YTick',y_ticks,'XTickLabel',f(y_ticks))
%     
%     if d==1
%         
%         title({[subject,', Channel ',num2str(channel)];'Normalized Spectral Power'})
