...
for n = 1 % 1:no_norms
    ...
    for s = 1 % :no_stats
        ...
        for d = 3 % 1:no_drugs
            ...
            plot_mean = [All_BP_stats(:, 1, 8, 2, d, n)]; % All_BP_stats(:, 2, 2, 2, d, n)];
            
            plot_se = [All_BP_stats(:, 1, 8, 3, d, n)]; % All_BP_stats(:, 2, 2, 3, d, n)];
            ...
            plot_mean = [All_BP_stats(:, 2, 8, 2, d, n)]; % All_BP_stats(:, 2, 1, 2, d, n)];
            
            plot_se = [All_BP_stats(:, 2, 8, 3, d, n)]; % All_BP_stats(:, 2, 1, 3, d, n)];
...
Frontal_BP = load([name, '/', name, '_BP_pct.txt']);
...
CA1_BP = load([name, '/', name, '_BP_pct.txt']);

BP = Frontal_BP; BP(:, :, 2) = CA1_BP;

chan_labels = {'Frontal', 'CA1'}; band_labels = {'HFO'};
...
for ch = 1:2
    ...
    for band = 8
        ...
        subplot(5, length(chan_labels)*length(band_labels), 4*length(chan_labels)*length(band_labels) + plot_count)
        
        % handle = bar([BP_median; nan nan]); 
        
        handle = barwitherr([BP_se; nan nan], [BP_mean; nan nan]);
        
        set(handle(1), 'FaceColor', colors{band - 7, ch}), set(handle(2), 'FaceColor', colors{band - 7, ch})
        ...
        title([chan_labels{ch}, ' ', band_labels{band - 7}])
