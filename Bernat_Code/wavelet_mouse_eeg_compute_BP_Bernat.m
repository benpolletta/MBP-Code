function wavelet_mouse_eeg_compute_BP_Bernat(subject,channel)
...
%     fprintf(fid_vec(1),make_format(no_f_bins,'f'),f);
    ...
    for i=1:no_bands
        fprintf(fid_BP,'%s\t',band_freq_labels{i});
    end
    fprintf(fid_BP,'%s\n','');
...
%     fprintf(fid_vec(1),spec_format,[(1:total_epochs)' drug_epoch_states spec_all]');
    fprintf(fid_BP,BP_format,[(1:total_epochs)' drug_epoch_states BP_all]');
...
%     BP_all_mean=ones(size(BP_all))*diag(nanmean(spec_all));
%     spec_all_std_inv=diag(1./nanstd(spec_all));
%     spec_all_norm=(spec_all-spec_all_mean)*spec_all_std_inv;
...
%     saveas(gcf,[all_dirname,'/',all_filename,'_',drugs{d},'_power.fig'])
    ...
    set(gca,'XTick',x_ticks,'XTickLabel',t(x_ticks))
    y_ticks=1:no_bands;
    set(gca,'YTick',y_ticks,'XTickLabel',band_labels_long)
