function PLV_multichannel_plots_w_profiles_time_series_by_state(state)
...
for n=1:no_norms
    ...
    for c=1:no_chan_pairs
        ...
        load(['ALL_',cp_labels{c},'_PLV/ALL_',cp_labels{c},'_PLV_',norms{n},'6mins_by_state_cplot_data.mat'])
        ...
        BP_ranksum_new = permute(BP_ranksum(:, state_index, :, 1:no_BP_hr_periods), [4 1 3 2]);
        
        BPr_dims = size(BP_ranksum_new);
        
        BP_ranksum_new = reshape(BP_ranksum_new, BPr_dims(1), 1, BPr_dims(2), BPr_dims(3));
        
        All_BP_ranksum(:, c, :, :, n) = BP_ranksum_new;
            ...
            for c = 1:no_chan_pairs
                ...
                for b = 1:no_bands
                    ...
                    if c == 1
                        ...
                        title({[drugs{d}, ', ', state]; [long_stats{s}, ' ', long_norms{n}, ' PLV']})
