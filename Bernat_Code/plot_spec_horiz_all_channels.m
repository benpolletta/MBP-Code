...
for i=1:3, xlabels{i}='Time Since Injection (h)'; ylabels{i}={channels{i};'Frequency (Hz)'}; end
...
for d=1:4
    ...
    for n=1:length(norms)
        ...
        for s=1:length(stats)
            ...
            titles = {[long_stats{s}, ' ', drugs{d}, ' Power ', long_norms{n},' ',drugs{d}]};
            
            figure_replotter_labels_subregion(1:3,3,1,clims,no_hr_ticks,no_freq_ticks,[(-40*6:6:0)+3,(0:6:12*10*6)-3],freqs,hour_lims,freq_lims,titles,xlabels,ylabels)
