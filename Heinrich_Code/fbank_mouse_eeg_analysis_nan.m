...
H=nan(16,no_challenges,2,5);
H_MI=nan(4,no_challenges,2,4);
...
bMI_graph_titles={'Thresholded MI (Empirical p-Value)';'Thresholded MI (Normal p-Value)';'Thresholded MI (Lognormal p-Value)';'Normal p-Value of MI';'Lognormal p-Value of MI'};

MI_graph_labels={'MI';'PV';'ZS';'LZS'};
bMI_graph_labels={'bMI_pt';'bMI_nt';'bMI_lt';'bMI_npv';'bMI_lpv'};
...
for j=1:no_challenges
    ...
    MI=nan(4,4,filenum,4);
    nMI=nan(16,16,filenum,5);
    max_bands=nan(filenum,2,4);
    max_nMI_bands=nan(filenum,2,5);
    times=nan(filenum,no_challenges);

    for k=1:filenum

        tic;

        [bands,MI(:,:,k,1),MI(:,:,k,2),MI(:,:,k,3),MI(:,:,k,4),nMI(:,:,k,1),nMI(:,:,k,2),nMI(:,:,k,3),nMI(:,:,k,4),nMI(:,:,k,5)]=CFC_September_fbank_nan(char(filenames(k)),600,5,20,100,.01,[4 12],16,[20 180],16,0,.9,'custom','Hz',dirnames,custom_bands);
