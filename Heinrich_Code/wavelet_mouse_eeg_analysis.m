function wavelet_mouse_eeg_analysis(challenge_list,challenge_name,challenge_descriptor,challenge_params,challenge_labels)
...
%     max_nMI_bands=zeros(filenum,2,5);
    ...
    for k=1:filenum
        ...
        [bands_lo,bands_hi,MI(:,:,k,1),MI(:,:,k,2),MI(:,:,k,3),MI(:,:,k,4),MI(:,:,k,5),MI(:,:,k,6),MI(:,:,k,7),MI(:,:,k,8),MI(:,:,k,9)]=CFC_September_wav(char(filenames(k)),600,20,100,.05,linspace(4.25,11.75,16)',linspace(25,175,16)','Hz',dirnames);
